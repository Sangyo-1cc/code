...
// 디버그 모드 - 사용자에게는 보이지 않도록 false로 설정
...
function saveToGoogleSheets() {
    if (typeof google !== 'undefined' && google.script && google.script.run) {
        const data = {
            ...
            totalScore: analyzeResults.scores.overall,
            depthScore: analyzeResults.scores.depth,
            backPosture: analyzeResults.scores.posture
        };

        google.script.run
            .withSuccessHandler(function(result) {
                log('데이터 저장 성공:', result);
            })
        ...
        log('Google Apps Script 환경이 아닙니다. 데이터가 저장되지 않습니다.');
    }
}
// 점수 애니메이션
...
async function downloadResult() {
    ...
    if (bestMomentCanvas) {
        // 비율 계산하여 9:16에 맞게 크롭
        const sourceWidth = bestMomentCanvas.width;
        const sourceHeight = bestMomentCanvas.height;
        ...
        // 배경 이미지 그리기
        ctx.drawImage(bestMomentCanvas, sx, sy, sw, sh, 0, 0, canvas.width, canvas.height);
    ...
    ctx.fillText('AI SQUAT COACH', canvas.width / 2, 120);
    // 왼쪽 정보 패널
    const leftPanelX = 60;
    const leftPanelY = 300;
    const panelWidth = 400;
    // 반투명 백그라운드 (roundRect 폴리필)
    ctx.fillStyle = 'rgba(0, 0, 0, 0.6)';
    drawRoundRect(ctx, leftPanelX, leftPanelY, panelWidth, 500, 30);
    ctx.fill();
    // 점수
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 120px Noto Sans KR';
    ctx.textAlign = 'center';
    ctx.fillText(analyzeResults.scores.overall, leftPanelX + panelWidth / 2, leftPanelY + 150);
    ctx.font = '30px Noto Sans KR';
    ctx.fillText('SCORE', leftPanelX + panelWidth / 2, leftPanelY + 200);
    // 운동 정보
    ctx.textAlign = 'left';
    ctx.font = '35px Noto Sans KR';
    const infoY = leftPanelY + 280;
    ctx.fillText(`🏃 ${squatCount}회 완료`, leftPanelX + 40, infoY);
    ctx.fillText(`📐 무릎각도 ${lowestKneeAngle.toFixed(0)}°`, leftPanelX + 40, infoY + 60);
    ctx.fillText(`💪 깊이점수 ${analyzeResults.scores.depth}점`, leftPanelX + 40, infoY + 120);
    // 오른쪽 피드백 패널
    const rightPanelX = canvas.width - leftPanelX - panelWidth;
    const feedbackY = 1000;
    // 주요 피드백 1개만 표시
    const mainFeedback = analyzeResults.feedback.find(f => f.type === 'motivation') || analyzeResults.feedback[0];
    if (mainFeedback) {
        // 피드백 백그라운드
        ctx.fillStyle = 'rgba(255, 255, 255, 0.15)';
        drawRoundRect(ctx, rightPanelX, feedbackY, panelWidth, 300, 30);
        ctx.fill();
        ...
        // 타이틀을 여러 줄로 나누기
        const words = mainFeedback.title.split(' ');
        let line = '';
        let y = feedbackY + 80;
        ...
        for (let n = 0; n < words.length; n++) {
            ...
            if (metrics.width > panelWidth - 80 && n > 0) {
                ctx.fillText(line, rightPanelX + panelWidth / 2, y);
                line = words[n] + ' ';
                y += 50;
            } else {
                line = testLine;
        ...
        const contentWords = mainFeedback.content.split(' ');
        line = '';
        y += 80;

        for (let n = 0; n < contentWords.length; n++) {
            const testLine = line + contentWords[n] + ' ';
            const metrics = ctx.measureText(testLine);
            if (metrics.width > panelWidth - 60 && n > 0) {
                ctx.fillText(line, rightPanelX + panelWidth / 2, y);
                line = contentWords[n] + ' ';
                y += 40;
            } else {
    ...
    ctx.textAlign = 'center';
    ctx.fillText(new Date().toLocaleDateString('ko-KR'), canvas.width / 2, canvas.height - 150);
    // 프라이버시 문구
    ctx.fillStyle = 'rgba(255, 255, 255, 0.6)';
    ctx.font = '22px Noto Sans KR';
    ctx.fillText('🔒 영상은 저장되지 않습니다', canvas.width / 2, canvas.height - 80);
    // 워터마크
    ctx.fillStyle = 'rgba(255, 255, 255, 0.3)';
    ctx.font = '20px Noto Sans KR';
    ctx.textAlign = 'right';
    ctx.fillText('#AISquatCoach', canvas.width - 50, canvas.height - 40);
    // 이미지 생성 및 다운로드
    canvas.toBlob((blob) => {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `squat_story_${new Date().getTime()}.png`;
...
async function shareToInstagram() {
    ...
    canvas.width = 1080;
    canvas.height = 1920;
    // 이미지 생성 로직 (downloadResult와 동일)
    // ... (위의 downloadResult 내용과 동일)
    canvas.toBlob(async (blob) => {
        const file = new File([blob], 'squat_result.png', { type: 'image/png' });
        ...
        if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
            try {
                await navigator.share({
                    files: [file],
                    title: 'AI 스쿼트 코치 결과',
                    text: `AI 스쿼트 분석 결과: ${analyzeResults.scores.overall}점! 🎯`
                });
                log('공유 성공');
            } catch (error) {
                log('공유 취소 또는 실패', error);
                // 공유 실패시 다운로드로 대체
                downloadResult();
            }
        } else {
            // Web Share API 미지원시 다운로드
            log('Web Share API 미지원');
            downloadResult();
            alert('이 기기에서는 직접 공유가 지원되지 않습니다. 다운로드된 이미지를 인스타그램에 업로드해주세요.');
        }
    }, 'image/png');
}
// 분석 초기화
