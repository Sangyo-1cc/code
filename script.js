...
let analysisTimeout = null; // 분석 타임아웃
let noSquatFrames = 0; // 스쿼트 미감지 프레임 카운터
...
squatData = [];
currentFrame = 0;
frameSkip = 0; // 프레임 스킵 카운터 추가
analysisTimeout = null; // 타임아웃 추가
noSquatFrames = 0; // 스쿼트 감지 실패 카운터
...
// 스쿼트 감지 실패 체크
if (noSquatFrames > 100) { // 약 10초간 스쿼트 미감지
...
```
// 에러 메시지를 분석 상태 텍스트에 표시
elements.analysisStatus.innerHTML = `<span style="color: #f44336;">⚠️ ${message}</span>`;
